...
import { MARKETGUIDE } from '@models/db_model';
...
export default function MarketGuide() {
  ...
  const [topic,setTopic]=useState('market_guide');
  ...
  const [ marketGuideList, setMarketGuideList] = useState<MARKETGUIDE[]>([]);
  const initialMarketGuideList = useRef<MARKETGUIDE[]>([]);
     ...
     (async () => {
         if(true){
             ...
             onValue(orderedRef, (snapshot) => {
               ...
               if (JSON.stringify(tempList) !== JSON.stringify(initialMarketGuideList.current)) {
                setMarketGuideList(tempList);
                initialMarketGuideList.current = tempList;
               }
  ...
  const handlePostButtonClick=()=>{
    ...
    let newMarketGuide:MARKETGUIDE={
      id:'',
      type:0,
      title:title,
      htmlContent:value,
    ...
    if(mediaFile===null){
      postObjectData(db_path,newMarketGuide,receiverRealTimeDbResults);
    }else{
      ...
      makeAnUploadToFirebasStorage(path,mediaFile,newMarketGuide,{setUploadProgress},receiverUploadResults);
    }
  ...
  const handleOnDelete=(item:MARKETGUIDE)=>{
  ...
  const sendNotificationAgain=(item:MARKETGUIDE)=>{
    if(dbuser==null){
      return;
    }
    ...
    <Layout>
      ...
      <main className="">
          ...
          <div className='rounded mx-5 mt-3'>
            ...
            {/**  View Data */}
            {tabNumber===1 && marketGuideList.length>0 && (
              <div className=''>
                {/* Table Here */}
              ...
              <div className='h-[400px] flex items-center justify-center'>
                <div className='mt-10 flex flex-col items-center justify-center'>
                  <div className='font-medium'>
                    You haven't posted market guides yet
                  </div>
                  <div>
